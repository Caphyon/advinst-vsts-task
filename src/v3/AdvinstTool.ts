...
const advinstMSBuildTargetsSubPath: string =
  'ProgramFilesFolder\\MSBuild\\Caphyon\\Advanced Installer';
const advinstDownloadUrlVar: string = 'advancedinstaller.url';
const advinstLicenseSubPath: string =
  'Caphyon\\Advanced Installer\\license80.dat';
const advinstIniUrlVar: string = 'advancedinstaller.ini.url';

const advinstWowRegKeyPath: string =
  'SOFTWARE\\Wow6432Node\\Caphyon\\Advanced Installer\\Installation Path';
const advinstRegKeyPath: string =
  'SOFTWARE\\Caphyon\\Advanced Installer\\Installation Path';

export async function getAdvinstComTool(): Promise<string> {
  const minAllowedVer = await _getMinAllowedAdvinstVersion();
  taskLib.debug(
    taskLib.loc('AI_DebugMinRequiredAdvinstVersion', minAllowedVer)
  );
  ...
  if (toolPath) {
    ...
    if (cmpVer.lt(toolVer, minAllowedVer) === 1) {
      throw new Error(
        taskLib.loc('AI_ErrorInvalidDetectedVersion', minAllowedVer, toolVer)
      );
    }
  ...
  if (cmpVer.lt(version, minAllowedVer) === 1) {
    throw new Error(
      taskLib.loc('AI_ErrorInvalidConfigVersion', minAllowedVer, version)
    );
  }
...
async function runAcquireAdvinst(version: string) {
  ...
  if (!version) {
    let cachedVersions: string[] = toolLib.findLocalToolVersions(
      advinstToolId,
      advinstToolArch
    );
...
async function getAdvinst(version: string, license: string): Promise<void> {
  ...
  let msBuildTargetsPath: string = path.join(
    cachedToolRoot,
    advinstMSBuildTargetsSubPath
  );
...
async function registerAdvinst(
  toolRoot: string,
  license: string
): Promise<void> {
  if (!license) return;
...
async function acquireAdvinst(version: string): Promise<string> {
  ...
  let cachedToolPath: string = await toolLib.cacheDir(
    advinstToolRoot,
    advinstToolId,
    semvish.clean(version),
    advinstToolArch
  );
...
async function _downloadAdvinst(version: string): Promise<string> {
  ...
  if (!advinstDownloadUrl) {
    advinstDownloadUrl =
      'https://www.advancedinstaller.com/downloads/' + version + '/advinst.msi';
  }
...
async function _extractAdvinst(sourceMsi: string): Promise<string> {
  ...
  let windowsInstallerFolder = path.join(
    taskLib.getVariable('windir'),
    'Installer'
  );
  ...
  if (!taskLib.exist(advinstWorkFolder)) taskLib.mkdirP(advinstWorkFolder);
  ...
  let exitCode = taskLib.execSync('msiexec.exe', msiexecArguments, {
...
function _getAdvinstToolVersion(path: string): string {
  ...
  const result = taskLib.execSync(path, ['/help'], {
    silent: true,
    outStream: stream
  });
...
async function _getUpdatesFileContent(): Promise<string> {
  const advinstIniUrl =
    taskLib.getVariable(advinstIniUrlVar) ||
